...
<script>
  // SOSTITUISCI QUESTO URL CON IL TUO URL REALE DI APPS SCRIPT
  ...
  // 2. Carica Tabella Segnalazioni Aperte
  ...
  // 3. Carica i dati delle colonne per il Menu Dinamico
  function loadMenu() {
  ...
  // 5. Gestione cambio Categoria Principale
  function onCategoriaChange() {
    ...
    contentSelect.innerHTML = '<option value="">-- Seleziona Prima Sotto-Categoria --</option>';
    
    if (!catValue) return;
    
    if (catValue === "SPORT") {
      var optB = rawMenuData["B"] ? rawMenuData["B"][0] : "Liste Eventi";
      var optD = rawMenuData["D"] ? rawMenuData["D"][0] : "Liste Canali";
      var optF = rawMenuData["F"] ? rawMenuData["F"][0] : "Roja Tube";
      var optG = rawMenuData["G"] ? rawMenuData["G"][0] : "Sport Replay";
      ...
      var opt = document.createElement("option");
      opt.value = "RADIO_ALL";
      opt.textContent = "Tutte le Radio";
      opt.selected = true;
      subSelect.appendChild(opt);
  ...
  // 6. Gestione cambio Sotto-Categoria
  function onSottoCategoriaChange() {
    ...
    if (subValue.indexOf("DIRECT:") === 0) {
      var nomeSel = subValue.replace("DIRECT:", "");
      var opt = document.createElement("option");
      opt.value = nomeSel;
      opt.textContent = nomeSel;
      opt.selected = true;
      contentSelect.appendChild(opt);
    
    } else if (subValue === "RADIO_ALL") {
  ...
  // 7. Invio del Form al Server Apps Script
